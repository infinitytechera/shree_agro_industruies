...
namespace shree_agro
{
    public static class HardwareInfo
    {
        ...
        public static String GetProcessorId(int start, int length)
        {
        ...
        public static String GetHDDSerialNo(int start, int length)
        {
            ManagementClass mangnmt = new ManagementClass("Win32_LogicalDisk");
            ManagementObjectCollection mcol = mangnmt.GetInstances();
            string result = "";
        ...
        public static string GetMACAddress()
        {
            ...
            try
            {
                ...
                using (Process process = Process.Start(info))
                {
                    ...
                    string[] mac = returnvalue.Split('\n');
                    address = mac[7].Substring(0, 17);
                ...
                if (address == "Disabled         ")
                {
                    using (Process process = Process.Start(info))
                    {
                        StreamWriter sw = process.StandardInput;
                        //StreamReader sr = process.StandardOutput;
                        sw.WriteLine("wmic path win32_networkadapter where PhysicalAdapter=True call enable");
                    ...
                    using (Process process = Process.Start(info1))
                    {
                        StreamWriter sw = process.StandardInput;
                        StreamReader sr = process.StandardOutput;
                        //string lan="Local Area Connection";
                        //sw.WriteLine("wmic path win32_networkadapter where PhysicalAdapter=True call enable");
                        sw.WriteLine("getmac ");
                        sw.Close();
                        returnvalue = sr.ReadToEnd();
                        ...
                        if (address == "Disabled         ")
                        {
                            MessageBox.Show("Please Run As Administrator.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            Application.Exit();
                        }
                        else
                        {
        ...
        public static string GetMACAddress(int start, int length)
        {
            string returnvalue = "";
            ProcessStartInfo info = new ProcessStartInfo("cmd.exe");
            info.UseShellExecute = false;
            ...
            //string[] mac = returnvalue.Split('=');
            string[] mac = returnvalue.Split('\n');
            string macaddress = mac[16].Substring(0, 18);
            macaddress = macaddress.Replace("-", "");
            macaddress = macaddress.Substring(start, length);
            return macaddress;
        }
        /// <summary>
        ...
        public static string GetMD5(string code, int start, int length)
        {
            string md5 = "";
            try
            {
                md5 = new CryptoLib().MD5Hash(code).Substring(start, length);
            }
            catch (Exception ex)
            {
            }
